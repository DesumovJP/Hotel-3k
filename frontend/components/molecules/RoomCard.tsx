...
import { Check, Plus, ArrowRight, Maximize2, Users, Waves, Wifi, ChevronLeft, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { duration, easeOutExpo } from "@/lib/motion";
import { generateHotelRoomSchema, toJsonLd } from "@/lib/seo";
...
export function RoomCard({
  ...
  const { handleKeyDown } = useArrowNavigation(room.images.length, {
  ...
  // Track view when hovered
  const trackViewOnce = useCallback(() => {
    if (!hasTrackedView) {
      trackViewContent("room", room.id, { name: room.name });
      setHasTrackedView(true);
    }
  }, [hasTrackedView, room.id, room.name]);
  ...
  const nextImage = () => setImageIndex((prev) => (prev + 1) % room.images.length);
  const prevImage = () => setImageIndex((prev) => (prev - 1 + room.images.length) % room.images.length);
  ...
  return (
    <motion.article
      ...
      tabIndex={0}
    >
      {/* HotelRoom Schema for SEO */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: toJsonLd(roomSchema) }}
      />
      {/* Image Container */}
      <div
        className={cn(
        ...
        aria-label={`${room.name} image gallery, ${imageIndex + 1} of ${room.images.length}`}
      >
        {/* Main Image */}
        <motion.div
        ...
        >
          <Image
            ...
            alt={`${room.name} - Image ${imageIndex + 1} of ${room.images.length}`}
            fill
            priority={priority}
            className="object-cover"
        ...
        {room.images.length > 1 && (
          <>
            <button
              ...
              className={cn(
                "absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full glass-200 flex items-center justify-center z-10 tap-target focus-visible-ring transition-opacity duration-150",
                isHovered ? "opacity-100" : "opacity-0"
              )}
              aria-label="Previous image"
            >
              <ChevronLeft className="w-4 h-4 text-ink" aria-hidden="true" />
            </button>
            <button
              onClick={(e) => { e.preventDefault(); nextImage(); }}
              className={cn(
                "absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full glass-200 flex items-center justify-center z-10 tap-target focus-visible-ring transition-opacity duration-150",
                isHovered ? "opacity-100" : "opacity-0"
              )}
              aria-label="Next image"
            >
              <ChevronRight className="w-4 h-4 text-ink" aria-hidden="true" />
            </button>
          </>
        )}
        ...
        {room.images.length > 1 && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10" role="tablist" aria-label="Image navigation">
            {room.images.slice(0, 4).map((_, i) => (
              <button
                ...
                className={cn(
                  "w-2 h-2 rounded-full transition-all duration-150 tap-target focus-visible-ring",
                  i === imageIndex
                    ? "bg-white scale-125"
        ...
        {onCompareToggle && (
          <motion.button
            ...
            className={cn(
              "absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center z-10 transition-colors duration-150 tap-target focus-visible-ring",
              isComparing
                ? "bg-gold text-white"
                : "glass-200 text-ink hover:bg-white"
            )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
          <div className="flex items-end justify-between">
            <div>
              ...
              <h3 id={`room-title-${room.id}`} className="font-display text-display-sm text-white mb-2">
                {room.name}
              </h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href={`/rooms/${room.slug}`}>
            <motion.button
              ...
              className={cn(
                "flex items-center gap-2 px-6 py-3 rounded-sm font-medium text-sm transition-all duration-200 ease-out tap-target focus-visible-ring",
                room.available
            ...
            >
              ...
              {room.available && <ArrowRight className="w-4 h-4" aria-hidden="true" />}
            </motion.button>
          </Link>
        </div>
      </div>
...
export function RoomCardSkeleton({ variant = "default" }: { variant?: "default" | "featured" }) {
  return (
    <div className={cn(
      "glass-card rounded-2xl overflow-hidden shadow-elevation-2",
      variant === "featured" && "md:col-span-2"
    )}>
      <div className={cn(
        "skeleton",
